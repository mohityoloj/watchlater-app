...
// ⬇️ PASTE THIS DIRECTLY BELOW ⬇️
function extractWhatsAppLabels(body: any): string[] {
  try {
    const msg =
    ...
    return labels
      .map((l: any) => l?.name)
      .filter(Boolean)
      .map((name: string) => `wa:${name.toLowerCase()}`);
  } catch {
    return [];
  }
}
...
export default async function handler(
  ...
  try {
    const body = req.body as any;
	 // 🔍 Add this here
  console.log("WHATSAPP RAW BODY:", JSON.stringify(body, null, 2));
    const message: string = body.Body || body.message || "";
	const sender = body.From || body.WaId || null;
...
`;
	...
	// Extract WhatsApp labels + merge with AI tags
	const whatsappLabels = extractWhatsAppLabels(body);
	const combinedLabels = [...whatsappLabels, ...tags];
