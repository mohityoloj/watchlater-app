...
// Helper: Fetch Instagram Post metadata via embedded JSON
...
// Helper: Fetch Instagram Reel metadata via window.__additionalDataLoaded
...
// Helper: Fetch generic OpenGraph metadata
...
export default async function handler(
  ...
  try {
    ...
    // ------------------------------------
    // 1. METADATA (YouTube, Instagram Post, Instagram Reel, OpenGraph)
    // ------------------------------------
    ...
    } else if (isInstagram(message)) {
      console.log("Detected Instagram URL — fetching Instagram metadata...");

      let meta = await fetchInstagramPostMetadata(message);

      if (!meta && message.includes("/reel/")) {
        console.log("Trying Reel metadata extractor...");
        meta = await fetchInstagramReelMetadata(message);
      }
